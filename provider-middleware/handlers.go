...
func (sh *ServiceHandler) initSubscription() error {
	subscriptions := []struct {
		...
		{models.ProgOverviewJob.PubName(), sh.handleProgramsOverviewRefresh},
	}
	for _, sub := range subscriptions {
		_, err := sh.nats.QueueSubscribe(sub.topic, "middleware", func(msg *nats.Msg) {
			ctx, cancel := context.WithTimeout(sh.ctx, CANCEL_TIMEOUT)
			defer cancel()
			sub.fn(ctx, msg)
		})
		if err != nil {
...
func (sh *ServiceHandler) handleInsertDailyProgHistory(ctx context.Context, msg *nats.Msg) {
	jobId, err := extractJobID(msg)
	if err != nil {
		logger().Errorf("failed to extract job_id: %v", err)
		return
	}
	success := sh.InsertDailyProgHistory(ctx) != nil
...
func (sh *ServiceHandler) handleProgramsOverviewRefresh(ctx context.Context, msg *nats.Msg) {
	logger().Infof("Refreshing Programs Overview")
	success := true
	jobId, err := extractJobID(msg)
	if err != nil {
		logger().Errorf("failed to extract job_id: %v", err)
	...
	if err := sh.refreshProgramOverviewViews(ctx); err != nil {
		logger().Errorf("error refreshing programs overview views: %v", err)
		success = false
	}
	sh.cleanupJob(ctx, nil, jobId, success)
}
