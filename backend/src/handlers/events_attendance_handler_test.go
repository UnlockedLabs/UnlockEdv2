...
func TestHandleGetAttendeesForSection(t *testing.T) {
	httpTests := []httpTest{
		{"TestGetAttendeesForSectionAsAdmin", "admin", nil, http.StatusOK, getQueryParamsForAttendees()},
		{"TestGetAttendeesForSectionAsUser", "student", nil, http.StatusUnauthorized, getQueryParamsForAttendees()},
	}
	for _, test := range httpTests {
		t.Run(test.testName, func(t *testing.T) {
			req, err := http.NewRequest(http.MethodGet, fmt.Sprintf("/api/program-sections/{id}/attendees%s", test.queryParams), nil)
			if err != nil {
				t.Fatalf("unable to create new request, error is %v", err)
			}
			req.SetPathValue("id", fmt.Sprintf("%d", 5)) //static...doesn't use 5
			handler := getHandlerByRoleWithMiddleware(server.handleGetAttendeesForSection, test.role)
...
func TestHandleLogAttendeeForEvent(t *testing.T) {
	...
	for _, test := range httpTests {
		t.Run(test.testName, func(t *testing.T) {
			...
			rr := executeRequest(t, req, handler, test)
			if test.expectedStatusCode == http.StatusOK {
				att := &models.ProgramSectionEventAttendance{
					EventID: eventId,
					UserID:  userId,
					Date:    req.URL.Query().Get("date"),
				}
				received := rr.Body.String()
				data := models.Resource[models.ProgramSectionEventAttendance]{}
				if err := json.Unmarshal([]byte(received), &data); err != nil {
					t.Errorf("failed to unmarshal resource, error is %v", err)
				}
				if diff := cmp.Diff(att, &data.Data, cmpopts.IgnoreFields(models.ProgramSectionEventAttendance{}, "ID", "CreatedAt", "UpdatedAt")); diff != "" {
...
func getQueryParamsForAttendees() string {
	attendances := []models.ProgramSectionEventAttendance{}
	if err := server.Db.Model(&models.ProgramSectionEventAttendance{}).Find(&attendances).Error; err != nil {
		return "" //do not crash
	}
