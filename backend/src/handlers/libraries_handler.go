...
func (srv *Server) handleGetQuerySuggestions(w http.ResponseWriter, r *http.Request, log sLog) error {
	...
	searchUrl := fmt.Sprintf("https://suggestqueries.google.com/complete/search?client=firefox&q=%s", escapedQuery)
	resp, err := http.Get(searchUrl)
	if err != nil {
		return newInternalServerServiceError(err, "error executing request to google suggest")
	}
