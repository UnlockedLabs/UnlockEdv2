...
func (srv *Server) registerLeftMenuRoutes() []routeDef {
	...
	return []routeDef{
		...
		{"POST /api/helpful-links/{id}/whitelist", srv.handleSubmitWhitelistRequest, true, axx},
	}
}
...
func (srv *Server) handleSubmitWhitelistRequest(w http.ResponseWriter, r *http.Request, log sLog) error {
	...
	emailReq := newWhitelistRequest(user, link)
