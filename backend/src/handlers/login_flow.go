...
func (srv *Server) handleRefreshAuth(w http.ResponseWriter, r *http.Request) error {
	...
	if !ok {
		...
		return newBadRequestServiceError(errors.New("no session found in form"), "Bad Request", fields)
	}
	...
	if err != nil { ///wait on this one
		return NewServiceError(err, http.StatusBadRequest, "invalid request, must include challenge", nil)
	}
