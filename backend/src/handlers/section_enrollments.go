...
func (srv *Server) registerProgramSectionEnrollmentssRoutes() {
	srv.Mux.Handle("GET /api/section-enrollments", srv.applyMiddleware(srv.handleIndexProgramSectionEnrollmentss))
...
func (srv *Server) handleIndexProgramSectionEnrollmentss(w http.ResponseWriter, r *http.Request, log sLog) error {
	facilityID := r.Context().Value(ClaimsKey).(*Claims).FacilityID
	log.add("facility_id", facilityID)
	page, perPage := srv.getPaginationInfo(r)
	total, enrollemnts, err := srv.Db.GetProgramSectionEnrollmentssForFacility(page, perPage, facilityID)
	if err != nil {
...
func (srv *Server) handleGetUserEnrollments(w http.ResponseWriter, r *http.Request, log sLog) error {
	...
	total, enrollemnts, err := srv.Db.GetProgramSectionEnrollmentssForUser(id, page, perPage)
	if err != nil {
		log.add("userId", id)
		return newDatabaseServiceError(err)
	}
