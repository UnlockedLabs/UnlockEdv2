...
func (srv *Server) HandleUserPrograms(w http.ResponseWriter, r *http.Request) error {
	userId, err := strconv.Atoi(r.PathValue("id"))
	if err != nil {
		return newInvalidIdServiceError(err, "user ID", nil)
	}
	if !srv.canViewUserData(r) {
		return newForbiddenServiceError(err, "You do not have permission to view this user's programs", nil)
	}
	order := r.URL.Query().Get("order")
