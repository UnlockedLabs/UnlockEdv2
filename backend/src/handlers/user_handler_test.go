		...
		t.Run(test.testName, func(t *testing.T) {
			...
			if test.expectedStatusCode == http.StatusOK {
				if test.mapKeyValues["dbErr"] != nil {
					t.Errorf("failed to get users from db, error is %v", test.mapKeyValues["dbErr"].(error))
				}
	...
	for _, test := range httpTests {
		t.Run(test.testName, func(t *testing.T) {
			...
			if test.expectedStatusCode == http.StatusCreated {
				...
				received := rr.Body.String()
				unmarshed := models.Resource[NewUserResponse]{}
				if err := json.Unmarshal([]byte(received), &unmarshed); err != nil {
					t.Errorf("failed to unmarshal user")
				}
				if diff := cmp.Diff(user, &unmarshed.Data.User); diff != "" {
...
func TestDeleteUser(t *testing.T) {
	...
	for _, test := range httpTests {
		t.Run(test.testName, func(t *testing.T) {
			...
			if test.expectedStatusCode == http.StatusNoContent {
				received := rr.Body.String()
				unmarshaled := models.Resource[struct{}]{}
				if err := json.Unmarshal([]byte(received), &unmarshaled); err != nil {
					t.Errorf("failed to unmarshal user")
				}
				if unmarshaled.Message != test.mapKeyValues["message"] {
					t.Errorf("handler returned wrong body: got %v want %v", unmarshaled.Message, test.mapKeyValues["message"])
				}
			}
		})
	}
...
func TestResetStudentPassword(t *testing.T) {
	...
	for _, test := range httpTests {
		t.Run(test.testName, func(t *testing.T) {
			...
			if test.expectedStatusCode == http.StatusOK {
				received := rr.Body.String()
				unmarshaled := models.Resource[map[string]string]{}
				if err := json.Unmarshal([]byte(received), &unmarshaled); err != nil {
					t.Errorf("failed to unmarshal user")
				}
				if unmarshaled.Data["message"] != test.mapKeyValues["message"] {
					t.Errorf("handler returned wrong body: got %v want %v", unmarshaled.Data["message"], test.mapKeyValues["message"])
				}
			}
		})
	}
