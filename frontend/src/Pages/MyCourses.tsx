        ...
        <AuthenticatedLayout title="My Courses">
            ...
            <div className="px-8 py-4">
                ...
                {Object.entries(TabType).map(([key, value]) => (
                    <button className={activeTab==TabType[key]? "text-teal-4 font-bold" :""} onClick={()=>setActiveTab(TabType[key])} key={Math.random()}>{value}</button>
                ))}
                </div>
                <div className="flex flex-row items-center mt-4 justify-between">
                    ...
                    <div className="flex flex-row items-center gap-2 body-small">
                        ...
                        <div className="bg-teal-1 join p-1">
                            ...
                            <button className={`flex gap-2 px-3 py-1 items-center rounded-lg ${activeView==ViewType.Grid && "bg-background"}`} onClick={() => setActiveView(ViewType.Grid)}><Squares2X2Icon className="h-4"></Squares2X2Icon> Grid</button>
                            <button className={`flex gap-2 px-3 py-1 items-center rounded-lg ${activeView==ViewType.List && "bg-background"}`} onClick={() => setActiveView(ViewType.List)}><ListBulletIcon className="h-4"></ListBulletIcon> List</button>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-4 gap-6 mt-8">
                    {recentCourses.map((course: any, index:number) => {
                        return (
                            <CourseCard course={course} status={course.status} favorited={course?.favorited} key={index} />
                        );
                    })}
                </div>
                :
                <div className="grid gap-4 mt-8">
                    {recentCourses.map((course: any, index:number) => {
                        return (
                            <CourseCardList course={course} status={course.status} favorited={course?.favorited} key={index} />
                        );
                    })}
                </div>
                }
