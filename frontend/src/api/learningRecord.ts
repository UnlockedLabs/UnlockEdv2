import API from './api';
import type { TranscriptDraft, TranscriptEntry, TranscriptQ4Toggle } from '@/types/digital-transcript';
...
function extractArray(resp: Awaited<ReturnType<typeof API.get<BackendEntry[]>>>): BackendEntry[] {
    if (!resp.success) return [];
    if (resp.type === 'many') return resp.data as BackendEntry[];
    if (Array.isArray(resp.data)) return resp.data as BackendEntry[];
    return [];
}
...
export async function apiCreateEntry(
    ...
    const b = resp.data as BackendEntry;
    return { entry: toFrontend(b), backendId: b.id };
}

export async function apiUpdateEntry(backendId: number, entry: TranscriptEntry): Promise<boolean> {
...
export async function apiDeleteEntry(backendId: number): Promise<boolean> {
    const resp = await API.delete<string>(`learning-record/entries/${backendId}`);
    return resp.success;
}

export async function apiGetDraft(): Promise<TranscriptDraft | null> {
    const resp = await API.get<BackendEntry & { updated_at: string }>('learning-record/draft');
    if (!resp.success || resp.type !== 'one' || !resp.data) return null;
