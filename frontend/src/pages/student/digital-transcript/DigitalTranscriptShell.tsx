 ...
 */
export function DigitalTranscriptShell({
    children,
    variant = 'wide'
}: ShellProps) {
    const innerClass =
        variant === 'narrow' ? 'mx-auto w-full max-w-xl' : 'w-full';
...
export function DigitalTranscriptEyebrow({
    children
}: {
    children: ReactNode;
}) {
...
export function DigitalTranscriptPageTitle({
    children
}: {
    children: ReactNode;
}) {
