...
export default function DigitalTranscriptEntryPage() {
    ...
    const { hydrated, upsertCommittedEntry, deleteCommittedEntry, entries } =
        useTranscriptDraft();
    const { user } = useAuth();
    ...
    const handleRegisterFunnelFinish = useCallback(
        (handlers: FunnelFinishHandlers) => {
            funnelFinishRef.current = handlers;
        },
        []
    );

    const handleFunnelAutoSaveStatusChange = useCallback(
        (state: FunnelAutoSaveState) => {
    ...
    const handleDownload = useCallback(async () => {
        const rows =
            liveExportRowsRef.current.length > 0
                ...
                : readLearningRecordExportRows(entries);
            ...
            <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
                ...
                >
                    {isFunnel ? (
                        ...
                        <DigitalTranscriptBackLink to={base}>
                            Back
                        </DigitalTranscriptBackLink>
                    )}
