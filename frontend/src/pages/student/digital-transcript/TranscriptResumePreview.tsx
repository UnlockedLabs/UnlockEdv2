...
function residentDisplayName(user: ReturnType<typeof useAuth>['user']): string {
    ...
    const first =
        typeof user.name_first === 'string' ? user.name_first.trim() : '';
    const last =
        typeof user.name_last === 'string' ? user.name_last.trim() : '';
    const full = [first, last].filter(Boolean).join(' ');
    return full.length > 0 ? full : 'Resident';
}

type PreviewSource = LearningRecordDocumentSource;
    ...
    return (
        ...
        >
            ...
            <CardContent
                className={cn(
                    isPaper
                        ? 'flex min-h-0 flex-1 flex-col px-0 py-4'
                        : 'space-y-8 pt-7'
                )}
            >
