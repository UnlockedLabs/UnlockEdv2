...
const skeletonBarClass = 'block rounded-md bg-muted/70';

function SkeletonBar({ className }: { className?: string }) {
    return <span className={cn(skeletonBarClass, className)} aria-hidden />;
}
...
export function LearningRecordDocument({
    ...
    const labels = isRecord
        ? LEARNING_RECORD_PREVIEW_LABELS
        : DOCUMENT_PREVIEW_LABELS;
    const emptyPh = emptyAnswerLabel?.trim();
    const skeletonEmpty =
        !filledSectionsOnly && emptyPreviewVariant === 'skeleton';
    ...
    const showSkills =
        !isFunnel && (!filledSectionsOnly || isSkillsSectionFilled(source));
    const showMetadataColumn =
        !isFunnel && (!filledSectionsOnly || hasFilledMetadataSections(source));
    ...
    const singleColumn =
        filledSectionsOnly &&
        showMetadataColumn !== showNarrativeColumn &&
        !isFunnel;
    ...
    return (
        <article
            className={cn(
                'learning-record-print-root flex min-h-0 flex-1 flex-col',
                className
            )}
            lang="en"
        >
                ...
                <div className="print:hidden mb-4 shrink-0">
                    <div className="mb-1.5 flex items-baseline justify-between gap-2 text-xs text-muted-foreground">
                        <span className="leading-snug">
                            Reflections you have added so far
                        </span>
                    ...
                    <div
                        ...
                        role="img"
                        aria-label={`${answered} of ${totalSlots} reflections added`}
                    >
                        <div
                ...
                >
                    {showMetadataColumn ? (
                        <div className="flex min-w-0 flex-col gap-5 md:pr-5">
                            {showProgram ? (
                                <section
                                    aria-labelledby="lr-doc-program"
                                    className="break-inside-avoid space-y-1.5"
                                >
                                    <SectionLabel id="lr-doc-program">
                                        {labels.program}
                                    </SectionLabel>
                                    <div className="text-[18px] font-medium leading-snug text-foreground">
                                        {programDisplayTitle ?? (
                                            <EmptySlot
                                                ...
                                                skeleton={
                                                    <SkeletonBar className="h-6 w-[88%] max-w-md" />
                                                }
                                            />
                                        )}
                                    </div>
                            ...
                            {showCompleted ? (
                                <section
                                    aria-labelledby="lr-doc-completed"
                                    className="break-inside-avoid space-y-1.5"
                                >
                                    <SectionLabel id="lr-doc-completed">
                                        {labels.completed}
                                    </SectionLabel>
                            ...
                            {showConfidence ? (
                                ...
                                >
                                    <SectionLabel id="lr-doc-confidence">
                                        {labels.confidence}
                                    </SectionLabel>
                                    <div
                                        className="flex gap-1"
                                        role="img"
                                        aria-label={`Confidence ${seg} out of 5`}
                                    >
                                        {[1, 2, 3, 4, 5].map((i) => (
                                            <div
                                                key={i}
                            ...
                            {showSkills ? (
                                <section
                                    aria-labelledby="lr-doc-skills"
                                    className="break-inside-avoid space-y-2"
                                >
                                    <SectionLabel id="lr-doc-skills">
                                        {labels.skills}
                                    </SectionLabel>
                                    {source.topSkills.length > 0 ? (
                                        <ul className="flex list-none flex-wrap gap-1.5 p-0">
                                            {source.topSkills.map(
                                                (skill, idx) => (
                                                    <li key={`${idx}-${skill}`}>
                                                        <Badge
                                                            variant="secondary"
