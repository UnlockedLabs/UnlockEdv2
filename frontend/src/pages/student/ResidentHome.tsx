...
const REFLECT_NUDGE_STORAGE_KEY =
    'unlocked.resident-home.reflect-nudge.dismissed';
...
function parsePreviewLearningState(
    ...
    if (raw === 'incomplete' || raw === 'resume')
        return 'returningWithIncomplete';
...
export default function ResidentHome() {
    ...
    const { entries, hydrated } = useTranscriptDraft();
    ...
    const learningRecordFormVariant = getLearningRecordFormVariant(
        DIGITAL_TRANSCRIPT_BASE
    );
    ...
    useEffect(() => {
        ...
        return () =>
            window.removeEventListener(
                'transcript-entry-session-updated',
                bump
            );
    }, []);
    ...
    }, [entries, entrySessionTick, learningRecordFormVariant]);
    ...
    const inProgressEntryReminder = useMemo(
        () =>
            incompleteEntry
                ? {
        ...
        <div className="bg-muted min-h-screen p-6" id="resident-home">
                    ...
                    {learningRecordEnabled && reflectNudgeVisible ? (
                        <Alert className="border-[#556830]/30 bg-[#556830]/5">
                            ...
                            <AlertDescription className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                <span>
                                    It has been a while since you logged an
                                    achievement. Add anything you have completed
                                    recently so your record stays current.
                                </span>
            ...
            {learningRecordEnabled ? (
                <ViewAllAchievementsSheet
                    ...
                    documentVariant={
                        learningRecordFormVariant === 'funnel'
                            ? 'funnel'
                            : 'default'
                    }
                />
