...
// The statewide master toggles (global feature_flags / page_feature_flags).
// Only system admins reach this section; the backend also rejects non-system
// admins on the toggle endpoints. It reads the acting system admin's own
// feature_access, which resolves to the global set since system admins are
// statewide.
export default function GlobalFeatureSection() {
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            if (!resp.success) {
                toast.error(
                    `Failed to ${newValue ? 'enable' : 'disable'} ${name}`
                );
                return;
            }
            const updated = await fetchUser();
            if (updated) setUser(updated);
            // A statewide change alters every facility's globally_enabled state,
            // so revalidate the per-facility section rendered below this one.
            await mutate(
                (key) =>
                    typeof key === 'string' &&
                    key.startsWith('/api/facilities/')
            );
            toast.success(
                `${name} ${newValue ? 'enabled' : 'disabled'} successfully`
            );
            setPending(null);
        } catch {
        ...
        <div className="space-y-4">
            <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                    Manage which features are available across the system
                </p>
            </div>
            ...
            <div className="bg-background border border-border rounded-lg p-6">
                <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground mt-1">
                            Provides educational content, videos, and resources
                            for residents
                        </p>
                    </div>
                    <Switch
                ...
                <div className="mt-4 bg-surface-hover rounded-md p-4">
                    <div className="flex items-start gap-2 mb-4 text-sm text-muted-foreground">
                        ...
                        <span>
                            Sub-features (only available when Knowledge Center
                            is enabled)
                        </span>
                    </div>
                    ...
                    <div className="space-y-4">
                        <SubFeatureRow
                            ...
                            enabled={isEnabled(
                                FeatureAccess.RequestContentAccess
                            )}
                        ...
                        <SubFeatureRow
                            ...
                            enabled={isEnabled(
                                FeatureAccess.HelpfulLinksAccess
                            )}
                            parentEnabled={kcEnabled}
                            onToggle={() =>
                                requestToggle(
            ...
            <div className="bg-background border border-border rounded-lg p-6">
                <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground mt-1">
                            Allows residents to log and track their learning
                            achievements and generate a personal learning record
                        </p>
                    </div>
                    <Switch
                        checked={lrEnabled}
            ...
            <div className="bg-background border border-border rounded-lg p-6">
                <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground mt-1">
                            Enables program management, class scheduling,
                            attendance tracking, and resident enrollment
                            functionality
                        </p>
                    </div>
                    <Switch
            ...
            <Dialog
                open={!!pending}
                onOpenChange={() => !loading && setPending(null)}
            >
                <DialogContent className="max-w-md">
                    <DialogHeader>
                        <DialogTitle>
                            {pending?.newValue ? 'Enable' : 'Disable'}{' '}
                            {pending?.name}?
                        </DialogTitle>
                        <DialogDescription>
                            {pending?.newValue ? (
                    ...
                    {isDisabling && affectsResidents && (
                        <div className="bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-800 rounded-md p-3 flex gap-2">
                            ...
                            <p className="text-sm font-medium text-amber-800 dark:text-amber-300">
                                This affects all facilities and residents
                                statewide.
                            </p>
                        </div>
                    )}
    ...
    return (
        ...
        >
            <div className="flex-1 pr-4">
                <h4 className="text-sm font-medium text-[#203622]">{label}</h4>
