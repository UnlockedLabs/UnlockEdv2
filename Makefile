...
init: ascii_art
	...
	go install github.com/pressly/goose/v3/cmd/goose@latest && go install github.com/air-verse/air@latest && cd frontend-v2 && yarn install && cd ..
...
install-dep: ascii_art
	...
	docker compose down && docker volume rm -f unlockedv2_node_modules && cd frontend-v2 && yarn add $(NAME) && cd .. && docker compose up $(BUILD_RECREATE)
...
build: ascii_art
	...
	docker buildx build . -f backend/Dockerfile --tag=$(ACCOUNT_ID)$(REPO)/unlockedv2:latest  && docker buildx build frontend-v2 -f frontend-v2/Dockerfile --tag=$(ACCOUNT_ID)$(REPO)/frontend:latest && docker buildx build . -f provider-middleware/Dockerfile --tag=$(ACCOUNT_ID)$(REPO)/provider_middleware:latest
